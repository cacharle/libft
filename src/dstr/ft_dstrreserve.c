...
/*   Updated: 2020/05/17 12:43:02 by charles          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** \param capacity  Minimum capacity - 1 required
** \return          Passed dynamic string or NULL on error
*/

t_ftdstr	*ft_dstrreserve(t_ftdstr *dstr, size_t capacity)
{
	size_t	new_capacity;
	char	*new_str;

	if (capacity < dstr->capacity - 1)
		return (dstr);
	new_capacity = dstr->capacity <= 1 ? 2 : dstr->capacity;
	while (capacity > new_capacity - 1)
